...
public class OAuthRequestTokenTask extends AsyncTask<Void, Void, Void> {
	 ...
	 * @param t_tweet
	 */
	public OAuthRequestTokenTask(Context context, OAuthConsumer consumer,OAuthProvider provider,int page) {
		...
		try {
	       ...
	       context.startActivity(intent);
			...
			else if(page==2)
			{
				...
				url = provider.retrieveRequestToken(consumer,Constants.OAUTH_CALLBACK_URL1);
				Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(url)).setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP
						| Intent.FLAG_ACTIVITY_NO_HISTORY| Intent.FLAG_FROM_BACKGROUND);
				context.startActivity(intent);
			}
...
//			context.startActivity(intent);
	...
	//		Toast.makeText(context, "Problem Logging In to Twitter.Try Later ",
