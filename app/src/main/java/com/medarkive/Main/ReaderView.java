...
public class ReaderView extends AdapterView<Adapter> implements GestureDetector.OnGestureListener, ScaleGestureDetector.OnScaleGestureListener, Runnable {
	...
	public void smartMoveForwards() {
		...
		Log.e("smart move forward", "smart move formard");
	...
	public void smartMoveBackwards() {
		...
		Log.e("smart move backward", "smart move backward");
		// The following code works in terms of where the screen is on the
		// views;
		// so for example, if the currentView is at (-100,-100), the visible
		// region would be at (100,100). If the previous page was (2000, 3000)
	...
	// mScroller.fling(0, 0, (int) velocityX, (int) velocityY, bounds.left,
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
		if (!mScaling) {
			...
			mYScroll -= distanceY;
			requestLayout();
//		Log.i("dis x", distanceX+"");
//		
//		Log.i("dis Y", distanceY+"");
		
		Log.v("Curr Y", mScroller.getCurrY()+"");
		
		Log.v("CurrX", mScroller.getCurrX()+"");
		}
		return true;
	}
	...
	public void onScaleEnd(ScaleGestureDetector detector) {
		...
		// Log.v("Scalling values mScale", mScale+"");
		// Log.v("Scalling values mXScroll", mXScroll+"");
		// Log.v("Scalling values mYScroll", mYScroll+"");
		mScaling = false;
	}
	...
	public boolean onTouchEvent(MotionEvent e) {
		...
		// if (mScale > 1.0f) {
		// EBOOKFragment.mPager.setScroll(false);
		// } else {
		// EBOOKFragment.mPager.setScroll(true);
		// }
		if (mScroller.isFinished()) {
			if (mScale > 1.0f) {
				Log.v("Ponits ", mChildViews.get(mCurrent).getWidth() + " == " + mScroller.getFinalX() + "");
//				if (mChildViews.get(mCurrent).getWidth() == e.getX() || mChildViews.get(mCurrent).getY() == e.getY()) {
//					EBOOKFragment.mPager.setScroll(true);
//				} else {
				
				if(EBOOKFragment.mPager!= null){
					EBOOKFragment.mPager.setScroll(false);
				}
//				}
			} else {
		...
		if ((e.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_UP) {
			...
			if (v != null) {
				if (mScroller.isFinished()) {
					...
					// if (mScale > 1.0f) {
					// if (mScroller.getFinalX() - mScroller.getCurrX() == 0 &&
					// mScroller.getFinalY() - mScroller.getCurrY() == 0) {
					// EBOOKFragment.mPager.setScroll(true);
					// } else {
					// EBOOKFragment.mPager.setScroll(false);
					// }
		...
		// Log.v("Current x axis - >", mScroller.getCurrX() + "");
		// Log.v("Current y axis - >", mScroller.getCurrY() + "");
		// Log.i("final x axis - >", mScroller.getFinalX() + "");
		// Log.i("final y axis - >", mScroller.getFinalY() + "");
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		if (mCurrent > 0) {
			...
			lv.layout((cvRight + cvLeft - lv.getMeasuredWidth()) / 2, cvTop - lv.getMeasuredHeight() - gap, (cvRight + cvLeft + lv.getMeasuredWidth()) / 2, cvTop - gap);
		...
		if (mCurrent + 1 < mAdapter.getCount()) {
			...
			rv.layout((cvRight + cvLeft - rv.getMeasuredWidth()) / 2, cvBottom + gap, (cvRight + cvLeft + rv.getMeasuredWidth()) / 2, cvBottom + rv.getMeasuredHeight() + gap);
	...
	public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
		...
		if (v != null) {
			...
			case MOVING_LEFT:
				...
				// EBOOKFragment.mPager.setScroll(true);
				// }
				// ((DisplayDataActivity)context).leftGesture(context);
				break;
			...
			case MOVING_RIGHT:
				...
				// EBOOKFragment.mPager.setScroll(false);
				// }
				// ((DisplayDataActivity)context).rigtGesture(context);
				break;
			}
			mScrollerLastX = mScrollerLastY = 0;
			// If the page has been dragged out of bounds then we want to spring
	...
	// // so add left to the measured width for the correct position
	// if (cv.getLeft() + cv.getMeasuredWidth() + cvOffset.x + GAP / 2 +
	// mXScroll < getWidth() / 2 && mCurrent + 1 < mAdapter.getCount()) {
	// postUnsettle(cv);
	// // post to invoke test for end of animation
	// // where we must set hq area for the new current view
	// mStepper.prod();
	...
	// lv.layout(cvLeft - lv.getMeasuredWidth() - gap, (cvBottom + cvTop -
	// lv.getMeasuredHeight()) / 2, cvLeft - gap, (cvBottom + cvTop +
	// lv.getMeasuredHeight()) / 2);
	// }
	//
	...
	// cvRight + rv.getMeasuredWidth() + gap, (cvBottom + cvTop +
	// rv.getMeasuredHeight()) / 2);
	// }
	//
	// invalidate();
