...
import com.medarkive.Main.Hit;
import com.medarkive.Main.MuPDFView;
...
public class MuPDFFragment extends Fragment implements FilePicker.FilePickerSupport {
	...
	private MuPDFCore openBuffer(byte buffer[], String magic) {
		...
		try {
			core = new MuPDFCore(this.getActivity(), buffer, magic);
			// New file: drop the old outline data
			OutlineActivityData.set(null);
		} catch (Exception e) {
			System.out.println(e);
	...
	public View onCreateView(final LayoutInflater inflater, final ViewGroup container, final Bundle savedInstanceState) {
		...
		if (core == null) {
			...
			if (core != null && core.needsPassword()) {
				requestPassword(inflater, savedInstanceState);
				return null;
			}
		...
		View layout = createUI(inflater, savedInstanceState);
	...
	public void requestPassword(final LayoutInflater inflater, final Bundle savedInstanceState) {
		...
		alert.setButton(AlertDialog.BUTTON_POSITIVE, getString(R.string.mupdflib_okay), new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int which) {
				if (core.authenticatePassword(mPasswordView.getText().toString())) {
					createUI(inflater, savedInstanceState);
				} else {
					requestPassword(inflater, savedInstanceState);
				}
			}
	...
	public View createUI(LayoutInflater inflater, Bundle savedInstanceState) {
		if (core == null)
			return null;
		...
		mDocView.setAdapter(new MuPDFPageAdapter(this.getActivity(), this, core));

		mSearchTask = new SearchTask(this.getActivity(), core) {
			@Override
			...
			mOutlineButton.setOnClickListener(new View.OnClickListener() {
				public void onClick(View v) {
					...
					if (outline != null) {
						...
						Intent intent = new Intent(MuPDFFragment.this.getActivity(), OutlineActivity.class);
	...
	private void reflowModeSet(boolean reflow) {
		mReflow = reflow;
		mDocView.setAdapter(mReflow ? new MuPDFReflowAdapter(this.getActivity(), core) : new MuPDFPageAdapter(this.getActivity(), this, core));
	...
	private void printDoc() {
		...
		Intent printIntent = new Intent(this.getActivity(), PrintDialogActivity.class);
	...
	private void showInfo(String message) {
		...
		if (currentApiVersion >= android.os.Build.VERSION_CODES.HONEYCOMB) {
			SafeAnimatorInflater safe = new SafeAnimatorInflater(this.getActivity(), R.animator.info, (View) mInfoView);
		} else {
	...
	private void makeButtonsView(LayoutInflater inflater) {
	...
	public void proofWithResolution(int resolution) {
		...
		Intent intent = new Intent(this.getActivity(), MuPDFFragment.class);
	...
	public void OnProofButtonClick(final View v) {
		...
		final PopupMenu popup = new PopupMenu(this.getActivity(), v);
	...
	public void OnSepsButtonClick(final View v) {
		if (isProofing()) {
			...
			final PopupMenu menu = new PopupMenu(this.getActivity(), v);
	...
	public void performPickFor(FilePicker picker) {
		mFilePicker = picker;
		Intent intent = new Intent(this.getActivity(), ChoosePDFActivity.class);
